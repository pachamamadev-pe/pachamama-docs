...
Este documento presenta **tres estrategias de migración** hacia la misma arquitectura objetivo Azure. Se diferencian en la velocidad de transición, el tiempo de costos dobles y el nivel de riesgo de impacto al servicio activo. La **Opción A — Migración por Olas** es la recomendada para minimizar el impacto operativo.
...
flowchart LR
    ...
    subgraph AzureCorp["☁️ Azure Corporativo — Tenant Pachamaxter"]
        direction TB
...
gantt
    ...
    Resource Group + Key Vault + AppInsights   :done,  w1a, 2026-04-10, 3d
    Static Web Apps (Admin + Landing)          :       w1b, 2026-04-13, 4d
    Dominios + Alertas de Costo               :       w1c, 2026-04-17, 2d
    Azure Functions + Storage + Service Bus    :       w1d, 2026-04-14, 5d

    section Wave 2 — Bases de Datos
    PostgreSQL: provision + migración + validar :      w2a, 2026-04-22, 7d
    Cosmos DB: provision + import + validar    :       w2b, 2026-04-27, 5d
    Redis: provision + import + validar        :       w2c, 2026-05-01, 4d
    Staging: APIs contra BDs Azure             :       w2d, 2026-05-04, 3d

    section Wave 3 — APIs → Container Apps
    Dockerfiles + ACR + GitHub Actions         :       w3a, 2026-05-07, 5d
    Deploy APIs en ACA (staging)               :       w3b, 2026-05-11, 3d
    Traffic split 10% → 50%                    :       w3c, 2026-05-14, 5d
    Traffic split 50% → 100%                   :       w3d, 2026-05-19, 5d

    section Wave 4 — Cutover
    Validación 72h — 100% Azure                :       w4a, 2026-05-24, 3d
    Desactivar legacy providers                :       w4b, 2026-05-27, 2d
    Stand-by 14 días                           :       w4c, 2026-05-29, 14d
    Eliminación legacy + documentación         :       w4d, 2026-06-12, 3d
```
...
| Proveedor / Servicio | Costo Actual Est./mes | Costo Azure /mes | Diferencia |
|---------------------|----------------------|-----------------|------------|
| Heroku (4 Eco/Basic dynos) | ~$20–40 | $0 (→ ACA) | -$20–40 |
| Vercel (2 proyectos) | ~$20–40 (Pro) | $9 (Static Web Apps) | -$11–31 |
| Railway (PostgreSQL) | ~$5–20 | $144.66 (Flexible Server) | +$124–140 |
| MongoDB Atlas | ~$0–9 (M0/M10) | $15 (Cosmos DB Serverless) | +$6–15 |
| Redis Labs | ~$0–7 (free/paid) | $55 (C1 Standard) | +$48–55 |
| Azure Personal (actual) | ~$15–30 | Incluido en total | — |
| **Total estimado actual** | **~$60–136/mes** | **~$334.94/mes** | **+$199–275/mes** |

> **Nota importante**: El incremento de costo se justifica por la transición de un entorno MVP/free-tier a un entorno de **producción real** con SLA, alta disponibilidad de PostgreSQL (servidor dedicado 24/7), Redis replicado y Container Apps con réplicas activas. Los costos actuales son posibles porque la mayoría de servicios usa tiers gratuitos o muy limitados (Heroku Eco, Railway free, MongoDB Atlas M0). La arquitectura Azure está dimensionada para **1,000 usuarios concurrentes en producción real**.
>
...
**Implementar Opción A — Migración por Olas**, comenzando el 10 de Abril de 2026, con la siguiente prioridad de ejecución:

1. **Hoy (Wave 1)**: Provisionar la fundación (Key Vault, App Insights, Static Web Apps). Es de cero riesgo y desbloquea la migración del frontend inmediatamente, eliminando la dependencia de Vercel y estableciendo el gobierno de secretos que toda la migración posterior necesita.

2. **Semanas 3–5 (Wave 2)**: Migrar las bases de datos con estrategia dual-read. Este es el paso más crítico y requiere mayor atención. La recomendación es hacerlo en días hábiles, con Railway/Atlas/Redis en standby durante 2 semanas adicionales como red de seguridad.

3. **Semanas 6–8 (Wave 3)**: Las APIs Java en Heroku son los servicios más fáciles de migrar gracias al modelo de containerización — solo requieren Dockerfile + variables de entorno. El traffic splitting progresivo elimina prácticamente todo el riesgo.

4. **Costo aumenta ~$200–275/mes** respecto al estado actual, lo que es esperado y correcto al pasar de tiers gratuitos/MVP a infraestructura de producción dimensionada para 1,000 usuarios. Este costo puede optimizarse post-migración con Reserved Instances ahorrando ~$50/mes adicionales en el año 2.
